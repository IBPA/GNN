""" Train Bidirectional RNN and test.
...
from keras.layers import Dense, SimpleRNN, Bidirectional, Lambda
...
def getModel(nDInput, nDOutput, alpha, nDepth):
    model = Sequential()
    model.add(Bidirectional(SimpleRNN(units=nDOutput,
                                      kernel_regularizer=l2(alpha), activation='sigmoid'),
                            input_shape=(nDepth, nDInput),
                            merge_mode='ave'))
