...
filename <- file.path(Sys.getenv("HOME"), "/mygithub/grnnPipeline/data/ecoli_med//aggr_all.csv")
df <- read.csv(filename, sep = ",")
df$arch <- str_split_fixed(df$prefix, "_", 3)[,1]
df <- df[df$arch != "avg",]
...
print(gPlot)
strFigFilename <- file.path(sprintf("./figure/low_res_mae_comparison_syn_n10.pdf") )
...
strFigFilename <- file.path(sprintf("./figure/low_res_pcc_comparison_syn_n10.pdf") )
ggsave(strFigFilename, gPlot, width=15, height = 10)
print(gPlot)
...
print(gPlot)
strFigFilename <- file.path(sprintf("./figure/low_res_mae_overall_box_syn_n10.pdf") )
ggsave(strFigFilename, gPlot, width=15, height = 10)
