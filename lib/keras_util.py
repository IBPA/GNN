...
def fitModel(model, input, output, layers_config, alpha, prefix, dir_path, fold_filename):
    ...
    if isCallHistory:
        history = LossHistory()
        model.fit(input, output, epochs=10000, batch_size=nM, callbacks=[history, earlystop], verbose=1)
        history.save(dir_path, fold_filename, layers_config, alpha, prefix)
    else:
        model.fit(input, output, epochs=10000, batch_size=nM, callbacks=[earlystop], verbose=0)
